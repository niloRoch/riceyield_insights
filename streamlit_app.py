...
def simulator_page(df, model, scaler, feature_names):
    ...
    with tab3:
        ...
        st.markdown("Descubra como variações nos parâmetros afetam a produção")
        
        # Seletor de parâmetro para análise de sensibilidade
        sensitivity_param = st.selectbox(
            "Parâmetro para análise:",
            ['Nitrogen', 'POTASH', 'PHOSPHATE', 'avg_rain'],
            help="Varia este parâmetro e mantém os outros fixos"
        )
        ...
        with col1:
            variation_range = st.slider(
                "Variação (%)",
                -50, 100, 50,
                help="Percentual de variação em relação ao valor médio"
            )
        ...
        with col2:
            n_points = st.slider(
                "Número de pontos",
                10, 50, 20,
                help="Quantidade de pontos para análise"
            )
        
        # Realizar análise de sensibilidade
        base_values = {
            'Nitrogen': df['Nitrogen'].mean(),
            'POTASH': df['POTASH'].mean(),
            'PHOSPHATE': df['PHOSPHATE'].mean(),
            'avg_rain': df['avg_rain'].mean()
        }
        
        base_value = base_values[sensitivity_param]
        min_val = base_value * (1 - variation_range/100)
        max_val = base_value * (1 + variation_range/100)
        
        param_values = np.linspace(min_val, max_val, n_points)
        sensitivity_predictions = []
        
        progress_sensitivity = st.progress(0)
        
        for i, param_val in enumerate(param_values):
            # Configurar valores
            current_values = base_values.copy()
            current_values[sensitivity_param] = param_val
            
            # Calcular features
            annual = current_values['avg_rain'] * 12
            npk_total = current_values['Nitrogen'] + current_values['POTASH'] + current_values['PHOSPHATE']
            
            input_sens = pd.DataFrame({
                'ANNUAL': [annual],
                'avg_rain': [current_values['avg_rain']],
                'Nitrogen': [current_values['Nitrogen']],
                'POTASH': [current_values['POTASH']],
                'PHOSPHATE': [current_values['PHOSPHATE']],
                'NPK_Total': [npk_total],
                'N_P_Ratio': [current_values['Nitrogen'] / (current_values['PHOSPHATE'] + 1)],
                'N_K_Ratio': [current_values['Nitrogen'] / (current_values['POTASH'] + 1)],
                'P_K_Ratio': [current_values['PHOSPHATE'] / (current_values['POTASH'] + 1)],
                'Water_Efficiency': [1.0],
                'Rain_Intensity': [current_values['avg_rain'] / (annual / 365)],
                'Fertility_Index': [npk_total / 100000],
                'Nitrogen_x_Rain': [current_values['Nitrogen'] * current_values['avg_rain']],
                'Optimal_Score': [(npk_total * current_values['avg_rain']) / 1000000]
            })
            
            for col in feature_names:
                if col not in input_sens.columns:
                    input_sens[col] = df[col].mean()
            
            input_sens = input_sens[feature_names]
            
            try:
                input_scaled_sens = scaler.transform(input_sens)
                pred_sens = model.predict(input_scaled_sens)[0]
                sensitivity_predictions.append(pred_sens)
            except:
                sensitivity_predictions.append(base_values[sensitivity_param])
            
            progress_sensitivity.progress((i + 1) / n_points)
        
        progress_sensitivity.empty()
        ...
        fig_sens.add_trace(go.Scatter(
            ...
            mode='lines+markers',
            name=f'Sensibilidade {sensitivity_param}',
            line=dict(color='#00C851', width=3),
            marker=dict(size=8, color='#00C851')
        ))
        
        # Linha de referência (valor base)
        base_pred = sensitivity_predictions[len(sensitivity_predictions)//2]  # Ponto médio
        fig_sens.add_hline(
            y=base_pred,
            line_dash="dash",
            line_color="white",
            annotation_text=f"Base: {base_pred:.0f} kg/ha"
        )
        
        # Área de incerteza
        pred_std = np.std(sensitivity_predictions)
        fig_sens.add_hline(
            y=base_pred + pred_std,
            line_dash="dot",
            line_color="rgba(255, 107, 53, 0.5)",
        ...
        fig_sens.update_layout(
            title=f"Análise de Sensibilidade - {sensitivity_param}",
            xaxis_title=f"{sensitivity_param} ({'kg/ha' if 'rain' not in sensitivity_param.lower() else 'mm/mês'})",
            yaxis_title="Produção Predita (kg/ha)",
            template='plotly_dark',
            height=500
        )
        ...
        # Calcular estatísticas de sensibilidade
        sensitivity_range = max(sensitivity_predictions) - min(sensitivity_predictions)
        param_range = max_val - min_val
        sensitivity_ratio = sensitivity_range / param_range
        ...
        with col1:
            st.metric(
                ...
                f"±{sensitivity_range/2:.0f} kg/ha",
                f"{(sensitivity_range/base_pred)*100:.1f}% do valor base"
            )
        
        with col2:
            st.metric(
                "Sensibilidade",
                f"{sensitivity_ratio:.2f}",
                "kg/ha por unidade de parâmetro"
            )
        
        with col3:
            correlation_sens = np.corrcoef(param_values, sensitivity_predictions)[0,1]
            st.metric(
                "Correlação",
                f"{correlation_sens:.3f}",
                "Força da relação linear"
            )
        
        # Insights da análise de sensibilidade
        if correlation_sens > 0.8:
            sens_insight = "🟢 ALTA SENSIBILIDADE POSITIVA"
            sens_desc = f"Aumentar {sensitivity_param} resulta em ganhos significativos de produção"
        elif correlation_sens < -0.8:
            sens_insight = "🔴 ALTA SENSIBILIDADE NEGATIVA"
            sens_desc = f"Cuidado: excesso de {sensitivity_param} pode prejudicar a produção"
        elif abs(correlation_sens) > 0.5:
            sens_insight = "🟡 SENSIBILIDADE MODERADA"
            sens_desc = f"{sensitivity_param} tem impacto moderado na produção"
        else:
            sens_insight = "⚪ BAIXA SENSIBILIDADE"
            sens_desc = f"{sensitivity_param} tem pouco impacto na produção nesta faixa"
        
        st.markdown(f"""
        <div class="insight-box">
            <div class="insight-title">{sens_insight}</div>
            {sens_desc}
            <br><br>
            <strong>Recomendação:</strong> {'Otimizar este parâmetro pode trazer grandes ganhos' if abs(correlation_sens) > 0.7 else 'Focar em outros parâmetros mais impactantes'}
        </div>
        """, unsafe_allow_html=True)
...
def show_footer():
    ...
    <div style="background: var(--card-bg); backdrop-filter: blur(20px); 
        ...
        <div style="display: flex; justify-content: center; align-items: center; margin-bottom: 1rem;">
            ...
            <h3 style="margin: 0; background: var(--gradient-success); 
        ...
        <p style="color: #B0B0B0; margin: 1rem 0; font-size: 1.1rem;">
        ...
        <div style="border-top: 1px solid rgba(255, 255, 255, 0.1); 
            ...
            <p style="color: #666; font-size: 0.9rem; margin: 0;">
                Desenvolvido para otimização agrícola sustentável | Portfolio Project 2025
            </p>
            <p style="color: #666; font-size: 0.8rem; margin: 0.5rem 0 0 0;">
                © 2025 - Análise de Dados Avançada para Agricultura de Precisão
            </p>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if __name__ == "__main__":
    ...
    # JavaScript adicional para interações
    st.markdown("""
    <script>
    // Adicionar efeitos de loading suaves
    document.addEventListener('DOMContentLoaded', function() {
        // Smooth scroll behavior
        document.documentElement.style.scrollBehavior = 'smooth';
        
        // Fade in elements on page load
        const elements = document.querySelectorAll('.metric-card, .insight-box, .status-card');
